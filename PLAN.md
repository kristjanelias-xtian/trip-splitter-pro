...
| 2026-02-25 | Repo root cleanup | Moved 11 audit/planning markdown files + 4 family refactor phase prompts + grafana-dashboard.json + 3 balance snapshot JSONs to `docs/`. Moved 3 SQL debug scripts to `scripts/`. Deleted 10 tracked sheet-audit screenshots, 14 untracked root PNGs, 48 untracked screenshots, 1 log file. Updated CLAUDE.md cross-references to `docs/` paths. Root now contains only config files, entry point, and project-level docs (README, CLAUDE, PLAN). |
...
| 2026-03-01 | Birthday easter egg | PRs #456, #458, #459, #462: Date-gated + UUID-gated birthday greeting card for one user (Mar 1–2, 2026). Floating card in `App.tsx` with framer-motion enter/exit, auto-dismiss after 6s. Self-removing after window. |
